...
AOSPBRANCH="refs/tags/android-13.0.0_r13"
...
git checkout $LOSBRANCH
git am --signoff < "$PATCHESDIR/frameworks_av/0001-1-2-libcameraservice-reset-to-a0a10c95a363a0df93da67.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0002-2-2-libcameraservice-massive-revert-to-Android-12-st.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0003-camera-Allow-to-use-boottime-as-timestamp-reference.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0004-libaudiohal-Bring-back-2.0-HAL.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0005-camera-Allow-devices-to-load-custom-CameraParameter-.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0006-Revert-Camera-Remove-old-recording-path-support.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0007-Camera-Restore-camera-HALv1-support-1-2.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0008-Camera-Add-extensions-to-CameraClient.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0009-nuplayer-Avoid-crash-when-codec-fails-to-load.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0010-camera-Don-t-segfault-if-we-get-a-NULL-parameter.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0011-libstagefright-Support-YVU420SemiPlanar-camera-forma.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0012-stagefright-omx-Don-t-signal-dataspace-change-on-leg.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0013-stagefright-ACodec-Resolve-empty-vendor-parameters-u.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0014-libstagefright-Free-buffers-on-observer-died.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0015-libstagefright-use-64-bit-usage-for-native_window_se.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0016-camera-include-Don-t-override-possible-overlayed-hea.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0017-Camera-check-metadata-type-before-releasing-frame.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0018-stagefright-add-changes-related-to-high-framerates-i.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0019-Camera-Add-support-for-preview-frame-fd.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0020-CameraSource.cpp-support-PIXEL_FORMAT_YUV420SP_NV21.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0021-libstagefright-Fix-memory-leak-due-to-lock-timeout.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0022-camera-media-Support-legacy-HALv1-camera-in-mediaser.patch"
git am --signoff < "$PATCHESDIR/frameworks_av/0023-CameraService-Only-warn-if-callingPid-should-be-reje.patch"
